...
$("#title").css({"font-style" : "italic", "background-color" : "red"});
...
$("h2").css({"color" : "orange", "margin-top" : "0"});
...
// $("#list-1>li").css({"display" : "inline", ":not" : ".cloned"});
// $("#list-1>li").css("display" , "hidden").show();
...
$("ul:parent").css("color" , "green ").show();//??
...
$("p:has(b:only-child)").css("font-size", "36px");

// for em in p which is the last child of type set color to green

$("p:has(em:nth-last-of-type(1))").css("color", "green");//?
...
$("div").eq(1).has("img").css({"float": "left", "border": "red"})
